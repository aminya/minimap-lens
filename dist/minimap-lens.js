...
class MinimapLens {
  ...
  async showLens(editor, layerY) {
    if (!editor) return;
    const minimap = this.minimap.minimapForEditor(editor);
    const [editorWidth, editorHeight] = this.calcEditorDims(minimap, editor); // check if should create lens
    ...
    if (shouldCreateLens) {
      ...
      const minimapDims = this.calcMinimapDims(minimap); // add editor and related values to the cache
      ...
      this.lensView = lensMap.lens; // show lens

      this.lensView.style.display = ''; // check if editor height has changed compared to cache

      const editorHeightHasChanged =
        this.lensMap.get(editor).editorDims.editorHeight !== editorHeight; // update minimapDims if editor height has changed
  ...
  prepareCreateLens(item, editorHeight, editorWidth) {
    ...
    itemView.style.width = `${editorWidth}px`; // Is this the best way to go?
  ...
  setLensPosition(minimap, editor, lens, layerY) {
    ...
    const lensTranslateY = layerY - halfLensHeight; // @NOTE: Calculate the "actual" editor height, which corresponds to what `minimapHeight` represents
